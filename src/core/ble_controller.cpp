 ...
 *
 * Implements ESP32-equivalent BLE functionality:
 * - Deterministic MAC address from DEVICE_NAME + BLE_PASSKEY (forces re-pair on change)
 * - MITM + Bonding security with passkey
 * - Same GATT UUIDs as ESP32 for Flutter app compatibility
 * 
...
int gattWriteCallback(uint16_t value_handle, uint8_t* buffer, uint16_t size) {
    ...
    if (value_handle == s_controlCharHandle && size > 0) {
        ...
        if (!s_firstWriteReceived) {
            ...
            Serial.println("[BLE] First write received - connection fully established");
        }
...
static void derive_random_address(uint8_t* addr) {
    ...
    snprintf(identity_str, sizeof(identity_str), "%s%d", DEVICE_NAME, BLE_PASSKEY);
    
    uint16_t identity_hash = crc16_le(0, (const uint8_t*)identity_str, strlen(identity_str));
    ...
    Serial.printf("[BLE] Derived address: %02X:%02X:%02X:%02X:%02X:%02X (hash: 0x%04X)\n",

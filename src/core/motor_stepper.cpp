 ...
 * @brief Stepper motor control with trapezoidal acceleration
 * 
 * Updated for MechaPico MCB which uses MCP23017 I2C GPIO expanders
 * for step/dir control instead of direct Pico GPIO pins.
 */
...
bool MotorStepper::init() {
    ...
    Serial.printf("[MotorStepper] Motor %d initialized (%s, %d microsteps, MCP23017)\n",
                  cfg_.index,
...
void MotorStepper::update(float dtSec) {
    ...
    if (mode_ == StepperMode::Velocity) {
        ...
        if (stepsRemaining == 0) {
            ...
            float decelDistance = (currentSpeed_ * currentSpeed_) / (2.0f * cfg_.acceleration);
    ...
    // Clamp to max speed
    if (currentSpeed_ > cfg_.maxSpeed) currentSpeed_ = cfg_.maxSpeed;
    if (currentSpeed_ < -cfg_.maxSpeed) currentSpeed_ = -cfg_.maxSpeed;
...
uint8_t MotorStepper::getIndex() const {
    return cfg_.index;
}

bool MotorStepper::isEnabled() const {
    return cfg_.enabled;
}

MotorType MotorStepper::getType() const {
...
void MotorStepper::setTargetSpeed(float stepsPerSec) {
    ...
    // Clamp to max speed
    if (targetSpeed_ > cfg_.maxSpeed) targetSpeed_ = cfg_.maxSpeed;
    if (targetSpeed_ < -cfg_.maxSpeed) targetSpeed_ = -cfg_.maxSpeed;
}
...
bool MotorStepper::isMoving() const {
    return fabsf(currentSpeed_) >= 1.0f;
}
